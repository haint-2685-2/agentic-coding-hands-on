-- Add `title` column to kudo (the "Danh hiệu" the sender awards the
-- recipient). Required field per the Viết Kudo Figma spec. Existing rows
-- are backfilled with empty string so the NOT NULL constraint passes; new
-- writes go through fn_create_kudo which enforces non-empty.

alter table public.kudo
  add column if not exists title text not null default ''
    check (char_length(title) <= 80);

-- Update fn_create_kudo signature to accept p_title (required, non-empty).
drop function if exists public.fn_create_kudo(uuid, text, text[], text[], text[], int[], boolean, text, uuid[]);
...
begin
  ...
  if char_length(coalesce(trim(p_title), '')) = 0 then
    raise exception 'validation/title_required' using errcode = 'P0004';
  end if;

  insert into public.kudo (sender_id, receiver_id, title, message, is_anonymous, anonymous_display_name, mentions)
  values (v_sender, p_receiver_id, trim(p_title), p_message, p_is_anonymous, p_anonymous_display_name, coalesce(p_mentions, '{}'::uuid[]))
...
grant execute on function public.fn_create_kudo(uuid, text, text, text[], text[], text[], int[], boolean, text, uuid[]) to authenticated;
